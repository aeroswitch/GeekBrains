...
class Scrapper():
    ...
    def __init__(self, mongodb_client, database, collection):
        ...
        self.superjob_url = 'https://www.superjob.ru'

        self.mongodb = MongoClient(mongodb_client)
        self.database = self.mongodb[database]
        self.collection = self.database[collection]

    def getting_html(self, link, params=None):
        html = requests.get(link, params=params, headers=self.headers)
        return html

    def getting_parsed_html(self, html):
        if html.ok:
            parsed_html = bs(html.text, 'html.parser')
            return parsed_html

    def searching_vacancy(self, vacancy_search_phrase):
        self.parsing_hh(vacancy_search_phrase)
        self.parsing_superjob(vacancy_search_phrase)

    def parsing_hh(self, vacancy_search_phrase):
        params = {
            'clusters': 'true',
        ...
        html = self.getting_html(self.hh_url, params)
        last_page
...
def parsing_vacanices():
    ...
    while True:
        ...
        for vacancy in vacancies_list:
            ...
            vacancy_salary = unicodedata.normalize('NFKD', vacancy_salary)
            vacancy_employer = vacancy.find('span',
                                            {'class': '_3mfro _3Fsn4 f-test-text-vacancy-item-company-name _9fXTd '
